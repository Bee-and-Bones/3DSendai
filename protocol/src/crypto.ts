...
// (which ag3nt adopts) carries nonce ‖ ciphertext ‖ mac as separate fields;
// the split/join lives in secure-frame.ts, so this module speaks libsodium's
// native "sealed = ciphertext‖mac" shape.

import _sodium from "libsodium-wrappers";
import { KEY_BYTES, NONCE_BYTES, MAC_BYTES } from "./crypto-constants.generated.ts";
...
function assertKeyNonce(key: Uint8Array, nonce: Uint8Array): void {
  if (key.length !== KEY_BYTES) throw new RangeError(`key must be ${KEY_BYTES} bytes, got ${key.length}`);
  if (nonce.length !== NONCE_BYTES) throw new RangeError(`nonce must be ${NONCE_BYTES} bytes, got ${nonce.length}`);
}

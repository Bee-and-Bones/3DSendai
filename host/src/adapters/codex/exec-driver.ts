...
import { normalizeCodexExec, extractThreadId, type CodexExecEvent } from "./exec-normalize.ts";
...
export class CodexExecAdapter implements Adapter {
  ...
  async prompt(text: string): Promise<void> {
    ...
    const args = this.threadId
      ? ["exec", "resume", this.threadId, "--json", "--skip-git-repo-check", "-C", this.opts.cwd, text]
      : [...base, text];

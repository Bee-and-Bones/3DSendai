...
import { splitTerminalHex, clampSize, type BridgeSink, type SessionBridge } from "../tmux/bridge.ts";
import {
  createHerdrClient,
  bootstrapHerdr,
  type HerdrClient,
...
export class HerdrBridge implements SessionBridge {
  ...
  private async bootstrap(): Promise<void> {
    try {
      ...
      const seed = (focusPane && this.byPane.get(focusPane)) || [...this.byId.values()].find((s) => !s.ended);
  ...
  route(type: number, sessionId: number, payload: unknown): void {
    if (this.phase !== "ready") {
      if (this.phase === "starting") this.pendingOps.push(() => this.route(type, sessionId, payload));
      return;
    }
    ...
    if (type === MSG.KEYSTROKE) {
      ...
      if (typeof p.hex !== "string" || p.hex.length === 0 || !/^([0-9a-fA-F]{2})+$/.test(p.hex)) return;
      const bytes: Uint8Array = fromHex(p.hex);
      // Verbatim: text and control bytes ride one ordered channel (U1 §7).
      this.child.write(
        JSON.stringify({ type: "terminal.input", bytes: Buffer.from(bytes).toString("base64"), encoding: "base64" }),
      );
      return;
    }
    if (type === MSG.CLIENT_SIZE) {
      ...
      this.child?.write(JSON.stringify({ type: "terminal.resize", cols: size.cols, rows: size.rows }));
    }
  }
  ...
  private openChannel(s: BridgeSession): void {
    ...
    try {
      ...
      this.child = undefined;
      this.emit(MSG.ERROR, s.id, { message: `herdr control channel failed: ${(err as Error).message}` });
      return;
    }
    this.child = child;
    let buf = "";
  ...
  private endSession(s: BridgeSession): void {
    if (s.ended) return;
    s.ended = true;
    this.emit(MSG.ALERT_SIGNAL, s.id, { sessionId: s.id, class: "session_ended" satisfies AlertClass });

    ...
    async request(method: string, params: unknown): Promise<Record<string, unknown>> {
      ...
      return await new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(
          () => finish(() => reject(new HerdrError("timeout", `herdr ${method}: no response in ${timeoutMs}ms`))),
          timeoutMs,
        );
        conn.onData(
          lineSplitter((line) => {
            let msg: { id?: string; result?: Record<string, unknown>; error?: { code: string; message: string } };
            try {
              msg = JSON.parse(line);
            } catch {
        ...
        conn.onClose(() => finish(() => reject(new HerdrError("closed", `herdr ${method}: connection closed before response`))));
        conn.write(JSON.stringify({ id, method, params }) + "\n");
      });
    },

    async subscribe(subscriptions: unknown[], handlers: SubscribeHandlers): Promise<HerdrSubscription> {
      ...
      return await new Promise((resolve, reject) => {
        ...
        conn.onData(
          lineSplitter((line) => {
            let msg: { id?: string; event?: string; data?: Record<string, unknown>; error?: { code: string; message: string } };
            try {
              msg = JSON.parse(line);
            } catch {
              return;
            ...
            if (typeof msg.event === "string") handlers.onEvent({ event: msg.event, data: msg.data ?? {} });
          }),
        );
        conn.onClose(() => {
          clearTimeout(timer);
          if (!acked) {
            reject(new HerdrError("closed", "herdr events.subscribe: connection closed before ack"));
            return;
          }
          if (!closed) {
            closed = true;
        ...
        conn.write(JSON.stringify({ id, method: "events.subscribe", params: { subscriptions } }) + "\n");
      });
    },
  };
}
...
export function parseSnapshot(result: Record<string, unknown>): HerdrSnapshot {
  ...
  if (result.type !== "session_snapshot" || !snap || !Array.isArray(snap.panes) || !Array.isArray(snap.workspaces) || !Array.isArray(snap.tabs)) {
    throw new HerdrError("bad_snapshot", "session.snapshot: response missing snapshot/panes — incompatible daemon");
  }
  for (const p of snap.panes) {
    if (!validatePane(p as Record<string, unknown>)) {
      throw new HerdrError("bad_snapshot", "session.snapshot: pane missing pane_id/workspace_id/tab_id/agent_status — incompatible daemon");
    }
  }
  return {
...
export async function bootstrapHerdr(client: HerdrClient): Promise<HerdrBootstrap> {
  ...
  if (protocol > HERDR_PROTOCOL) {
    warnings.push(`herdr daemon speaks protocol ${protocol} (${version}), newer than pinned ${HERDR_PROTOCOL}; continuing on capability check`);
  }
